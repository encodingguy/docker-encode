...
def Depth(input, depth=None, sample=None, fulls=None, fulld=None, \
    ...
    if dSType == sSType and dbitPS == sbitPS and (
            sSType == vs.FLOAT or (fulld == fulls and not prefer_props_range)) and not lowDepth:
        return clip

    # Apply conversion
    if useZ:
        clip = zDepth(clip, sample=dSType, depth=dbitPS, range=fulld, range_in=fulls, dither_type=dither,
                      prefer_props=prefer_props_range)
    else:
        clip = core.fmtc.bitdepth(clip, bits=dbitPS, flt=dSType, fulls=fulls, fulld=fulld, dmode=dither, ampo=ampo,
                                  ampn=ampn, dyn=dyn, staticnoise=staticnoise)
...
def ToRGB(input, matrix=None, depth=None, sample=None, full=None, \
    ...
    if sIsRGB:
        ...
        if sHSubS != 1 or sVSubS != 1:
            clip = core.fmtc.resample(clip, kernel=kernel, taps=taps, a1=a1, a2=a2, css="444", planes=[2, 3, 3],
                                      fulls=fulls, fulld=fulls, cplace=cplace, flt=pSType == vs.FLOAT)
        ...
        if matrix == "OPP":
            clip = core.fmtc.matrix(clip, fulls=fulls, fulld=fulld,
                                    coef=[1, 1, 2 / 3, 0, 1, 0, -4 / 3, 0, 1, -1, 2 / 3, 0], col_fam=vs.RGB)
            clip = SetColorSpace(clip, Matrix=0)
        elif matrix == "2020cl":
...
def ToYUV(input, matrix=None, css=None, depth=None, sample=None, full=None, \
    ...
    if sIsYUV or sIsYCOCG:
        ...
        if dHSubS != sHSubS or dVSubS != sVSubS:
            # Apply depth conversion for processed clip
            clip = Depth(clip, pbitPS, pSType, fulls, fulls, dither, useZ, prefer_props, ampo, ampn, dyn, staticnoise)
            clip = core.fmtc.resample(clip, kernel=kernel, taps=taps, a1=a1, a2=a2, css=css, planes=[2, 3, 3],
                                      fulls=fulls, fulld=fulls, cplace=cplace)
        # Apply depth conversion for output clip
        clip = Depth(clip, dbitPS, dSType, fulls, fulld, dither, useZ, prefer_props, ampo, ampn, dyn, staticnoise)
    elif sIsGRAY:
        # Apply depth conversion for output clip
        ...
        if matrix == "OPP":
            ...
            clip = core.fmtc.matrix(clip, mat=matrix, fulls=fulls, fulld=fulld,
                                    col_fam=vs.YCOCG if matrix == "YCgCo" else vs.YUV)
        # Change chroma sub-sampling if needed
        if dHSubS != sHSubS or dVSubS != sVSubS:
            clip = core.fmtc.resample(clip, kernel=kernel, taps=taps, a1=a1, a2=a2, css=css, planes=[2, 3, 3],
                                      fulls=fulld, fulld=fulld, cplace=cplace)
        # Apply depth conversion for output clip
        clip = Depth(clip, dbitPS, dSType, fulld, fulld, dither, useZ, prefer_props, ampo, ampn, dyn, staticnoise)
...
def BM3D(input, sigma=None, radius1=None, radius2=None, profile1=None, profile2=None, \
         ...
         block_size1=None, block_step1=None, group_size1=None, bm_range1=None, bm_step1=None, ps_num1=None,
         ps_range1=None, ps_step1=None, th_mse1=None, hard_thr=None, \
         block_size2=None, block_step2=None, group_size2=None, bm_range2=None, bm_step2=None, ps_num2=None,
         ps_range2=None, ps_step2=None, th_mse2=None):
    # Set VS core and function name
    core = vs.get_core()
    funcName = 'BM3D'
    clip = input
    ...
    if sIsGRAY:
        ...
        clip = ToRGB(clip, matrix, pbitPS, pSType, fulls, \
                     dither, useZ, prefer_props, ampo, ampn, dyn, staticnoise, cu_kernel, cu_taps, cu_a1, cu_a2,
                     cu_cplace, False)
        if pre is not None:
    ...
    if ref is not None:
        ...
        flt = core.bm3d.VBasic(clip, ref=pre, profile=profile1, sigma=sigma, radius=radius1, \
                               block_size=block_size1, block_step=block_step1, group_size=group_size1, \
                               bm_range=bm_range1, bm_step=bm_step1, ps_num=ps_num1, ps_range=ps_range1,
                               ps_step=ps_step1, \
                               th_mse=th_mse1, hard_thr=hard_thr, matrix=100).bm3d.VAggregate(radius=radius1,
                                                                                              sample=pSType)
    ...
    for i in range(0, refine):
        if skip:
            ...
            flt = core.bm3d.VFinal(clip, ref=flt, profile=profile2, sigma=sigma, radius=radius2, \
                                   block_size=block_size2, block_step=block_step2, group_size=group_size2, \
                                   bm_range=bm_range2, bm_step=bm_step2, ps_num=ps_num2, ps_range=ps_range2,
                                   ps_step=ps_step2, \
    ...
    if sIsGRAY:
        ...
        if output <= 0 and not sIsRGB:
            ...
            clip = ToYUV(clip, matrix, css, dbitPS, dSType, fulld, \
                         dither, useZ, prefer_props, ampo, ampn, dyn, staticnoise, cd_kernel, cd_taps, cd_a1, cd_a2,
                         cd_cplace)
        else:
    ...
    if tcfile is None:
        ...
        for clip in clips:
            if index > 0 and clip.fps_num == tc_list[index - 1][2] and clip.fps_den == tc_list[index - 1][3]:
                tc_list[index - 1] = (
                tc_list[index - 1][0], cur_frame + clip.num_frames - 1, clip.fps_num, clip.fps_den)
            else:
...
def PlaneCompare(clip1, clip2, plane=None, mae=True, rmse=True, psnr=True, cov=True, corr=True):
    ...
    if rmse or psnr:
        ...
        def _PlaneRMSEnPSNRTransfer(n, f):
            ...
            if psnr:
                fout.props.PlanePSNR = min(10 * math.log(valueRange * valueRange / f[1].props.PlaneMSE, 10),
                                           99.99999999999999) if f[1].props.PlaneMSE > 0 else 100.0
            return fout
    ...
    if cov or corr:
        ...
        CoDclip = core.std.FrameEval(floatBlk, functools.partial(_PlaneCoDFrame, clip1=clip1Plane, clip2=clip2Plane),
                                     [clip1Mean, clip2Mean])
...
def LimitFilter(flt, src, ref=None, thr=None, elast=None, brighten_thr=None, thrc=None, force_expr=None, planes=None):
    ...
    if planes is None:
        ...
        if planes < 0 or planes >= VSMaxPlaneNum:
            raise ValueError(
                funcName + ': valid range of \"planes\" is [0, {VSMaxPlaneNum})!'.format(VSMaxPlaneNum=VSMaxPlaneNum))
        process[planes] = 1
        ...
        for p in planes:
            if not isinstance(p, int):
                ...
                raise ValueError(funcName + ': valid range of \"planes\" is [0, {VSMaxPlaneNum})!'.format(
                    VSMaxPlaneNum=VSMaxPlaneNum))
            process[p] = 1
    else:
        raise TypeError(funcName + ': \"planes\" must be a (list of) int!')
...
def CheckMatrix(clip, matrices=None, full=None, lower=None, upper=None):
    ...
    def _FrameProps(n, f):
        fout = f.copy()
        fout.props.__setattr__(props[3], (
...
def CheckVersion(version, less=False, equal=True, greater=True):
    ...
    if not less and MvsFuncVersion < version:
        raise ImportWarning(
            'mvsfunc version(={0}) is less than the version(={1}) specified!'.format(MvsFuncVersion, version))
    if not equal and MvsFuncVersion == version:
        raise ImportWarning(
            'mvsfunc version(={0}) is equal to the version(={1}) specified!'.format(MvsFuncVersion, version))
    if not greater and MvsFuncVersion > version:
        raise ImportWarning(
            'mvsfunc version(={0}) is greater than the version(={1}) specified!'.format(MvsFuncVersion, version))
...
def zDepth(clip, sample=None, depth=None, range=None, range_in=None, dither_type=None, cpu_type=None,
    ...
    if zimgResize:
        clip = core.resize.Bicubic(clip, format=format.id, range=range, range_in=range_in, dither_type=dither_type,
                                   prefer_props=prefer_props)
    elif zimgPlugin and core.z.get_functions().__contains__('Format'):
        clip = core.z.Format(clip, format=format.id, range=range, range_in=range_in, dither_type=dither_type,
                             cpu_type=cpu_type)
    elif zimgPlugin and core.z.get_functions().__contains__('Depth'):
        clip = core.z.Depth(clip, dither=dither_type, sample=sample, depth=depth, fullrange_in=range_in,
                            fullrange_out=range)
    else:
...
def Preview(clips, plane=None, compat=None, matrix=None, full=None, depth=None, \
    ...
    def _Conv(clip):
        ...
        return core.resize.Bicubic(clip, ref.width, ref.height, format=dFormat,
                                   matrix_in=GetMatrix(clip, matrix, True, True), range_in=full, filter_param_a=0,
                                   filter_param_b=0.5, resample_filter_uv=kernel, filter_param_a_uv=a1,
                                   filter_param_b_uv=a2, dither_type=dither, prefer_props=prefer_props)
    ...
    if arg is None:
        ...
        if arg:
            if not arg.isidentifier():
                raise ValueError(
                    funcName + ': {argName}=\"{arg}\" is not a valid identifier!'.format(argName=argName, arg=arg))
        else:
            arg = False
    else:
...
def _limit_filter_expr(defref, thr, elast, largen_thr, value_range):
    ...
    if thr <= 0 and largen_thr <= 0:
        ...
        if thr <= 0:
            ...
            limitExpr = limitExpr.format(dif=dif, dif_abs=dif_abs, thr_1=thr_1, thr_2=thr_2, thr_slope=thr_slope,
                                         flt=flt, src=src)
        ...
        if largen_thr != thr:
            if largen_thr <= 0:
                ...
                limitExprLargen = " {dif_abs} {thr} <= {flt} {src} ? ".format(dif_abs=dif_abs, thr=largen_thr, flt=flt,
                                                                              src=src)
            else:
                thr_1 = largen_thr
                ...
                limitExprLargen = limitExprLargen.format(dif=dif, dif_abs=dif_abs, thr_1=thr_1, thr_2=thr_2,
                                                         thr_slope=thr_slope, flt=flt, src=src)
